...
def encode(data):
    id_features = ['shop_id', 'item_id', 'user_id', 'item_brand_id', 'item_city_id', 'user_gender_id',
                   ...
                   'user_occupation_id', 'context_page_id', 'top1', 'top2', 'top3', 'top4', 'top5', 'top10', 'query1',
                   'query', 'cate']
...
if os.path.exists('../data/origion_concat.csv'):
    ...
    train = pd.read_csv('../data/round2_train.txt', sep=' ', nrows=2000000)
    ...
    def f(x):
        try:
            return len([i for i in reduce((lambda x, y: x + y),
                                          [i.split(':')[1].split(',') for i in x.split(';') if len(i.split(':')) > 1])
                        if i != '-1'])
        except:
            return 0
    ...
    data['query'] = data['predict_category_property'].apply(
